...
@section('body')
    <div class='flex flex-col place-self-center bg-white p-5 rounded-xl shadow-xl mt-20'>
        <div class='flex flex-col items-center'>
            <img src='{{ env('IMG') }}/icons/pengelola.png' class='w-20' />
            <h1 class='text-center bg-black p-2 text-white mt-3 rounded-xl'>Pengelola Aset Input</h1>
        </div>

        <form action='{{ url('/admin/pengelola/input/data') }} ' method='post' class='mt-5 flex flex-col'>
            @csrf
        ...
        <script>
            if("{{ session('gagal') }}"){
                Swal.fire({
                    title: 'Gagal',
                    text: 'Data Pengelola Sudah Terdaftar',
                    icon: 'error'
                })
    ...
    <a href='{{ url('/admin/pengelola') }} ' class='flex place-self-center mt-2 bg-yellow-600 p-2 rounded-xl'>Kembali</a>
@endsection
